...
const ViewModal = ({
  ...
  console.log(
    'created_at parsing',
    created_at != '' || created_at != null ? new Date(created_at).toLocaleString('en-US') : ''
  );
  const organizationData = [
    ...
    {
      name: 'Registered On:',
      value: created_at != '' ? new Date(created_at).toLocaleString('lookup') : '',
    },
  ];
    ...
    >
      <Box className={classes.styles}>
        <Box className={classes.header}>
          <Typography variant="h5" component="div" sx={{ fontSize: '2rem', fontWeight: '700' }}>
